...
export function RapportZPage() {
	...
	// OUVRIR LE RAPPORT DE LA JOURNÉE — pas le fabriquer (31 août 2026).
	//
	// Le Z est émis par la route de clôture, et il est déjà dans le cache :
	// useCloseCashSession y a posé la réponse du serveur (lib/queries/cash.ts).
	// Il n'y a donc plus ni génération ni attente de `checkResult` : on demande
	// l'affichage, et useZReport rend l'entrée déjà présente.
	//
	// L'effet précédent attendait six conditions avant d'appeler handleGenerate.
	// C'est cette chaîne qui faisait dépendre un document fiscal d'un rendu.
	const aOuvert = useRef(false)
	useEffect(() => {
		if (!search.afficher) return
		if (aOuvert.current) return
		if (!selectedRegisterId || selectedRegisterId !== search.register) return
		if (selectedDate !== search.date) return

		aOuvert.current = true
		setShouldGenerate(true)

		// Nettoie les search params : un retour arrière ne doit pas rouvrir.
		navigate({
			to: '/cash/rapport-z',
			search: () => ({}),
			replace: true,
