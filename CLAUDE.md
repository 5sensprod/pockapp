...
- **Il n'y a plus qu'un écran catalogue**, `/stock/produits`, sur PocketBase
  ...
  **Les images des produits sont servies par PocketBase**, par
  ...
  (`frontend/modules/stock/single-source.test.ts`).
- **Le slug est posé au premier enregistrement publié, puis figé** (11 septembre 2026).
  Un produit créé en brouillon reste sans slug. `frontend/lib/queries/slug.ts`
  le dérive du nom lors du premier enregistrement en `published`, l'unicité
  ...
  produit l'AFFICHE en lecture seule, et enregistrer une fiche publiée qui n'en a pas
  la répare — il faut alors la ré-exporter. La publication en lot suit la même règle.
  Les slugs déjà attribués aux brouillons sont conservés. Gardiens : `slug.test.ts`,
  `product-publication.test.ts` et
