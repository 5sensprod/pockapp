...
func main() {
	...
	if r.GetString("status") == "published" {
		...
		fmt.Printf("  La ligne distante nommée %s restera en ligne, avec son slug,\n", ancien)
		fmt.Println("  et deviendra orpheline : plus aucun produit local ne la désigne.")
		fmt.Println("  Le prochain export présentera une entité NEUVE, dont le serveur")
		fmt.Println("  protégera le slug déjà pris (products-sync.php n'invente jamais")
		fmt.Println("  de slug et ne réattribue pas un slug existant).")
		fmt.Println("  → prévoir le ménage côté SQL distant.")
		fmt.Println()
	}
